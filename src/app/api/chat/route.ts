      ...
      try {
        ...
        if (properties && properties.length > 0) {
          propertyContext = `\n\nAvailable Properties:\n${properties
            .map((p) =>
              `- ${p.title} (${p.property_type}) in ${p.city}${p.area ? ", " + p.area : ""}: ₹${Number(p.price).toLocaleString()}, ${p.bedrooms ?? "?"}BR/${p.bathrooms ?? "?"}BA`
            )
            .join("\n")}`;
        }
      } catch {
        // silently skip if property fetch fails
      }
    }
...
- When a user wants to book a site visit or submit an enquiry, respond with a booking suggestion
...
- Don't make up property details not in the provided list${propertyContext}`;
    ...
    return NextResponse.json({
      reply,
      action: hasAction ? "book_visit" : undefined,
    });
  } catch (error: unknown) {
